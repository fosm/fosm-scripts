...
mapNode(qsItem,bbox)	   ; Process a single quadtree
	...
	. s continue=$$node(nodeId,"*","*",.bbox,1,qsItem) i 'continue s nodeId="" q
	. ;
	. s wayId=""
	. f  d  i wayId="" q
...
elementNode(k,v,qsItem,bbox)	; Process a single quadtree
	...
	. ;
	. s continue=$$node(id,k,v,.bbox,1,qsItem) i 'continue s id="" q
	;
	i 'continue q 0
	q 1
	
	
node(nodeId,k,v,bbox,relations,qsItem)	; Add node and all it's relations to workfile
	;
	n relationId,continue
	;
	i $d(^osmTemp($j,"node",nodeId)) q 1
	i 'gOrderedNodes w $$xml^node(indent,nodeId,gNodeElements,$g(qsItem))
...
relation(relationId,k,v,bbox)	; Add relation and all it's constituent elements to workfile
	...
	i $d(^osmTemp($j,"relation",relationId)) q 1
	s ^osmTemp($j,"relation",relationId)=""
	s continue=$$elementCounter i 'continue q 0
	;
...
wayInBox(wayId,bbox)	; Is a way within the bbox?
	...
	; 
	n way,qsBox,bllat,bllon,trlat,trlon
	s way=^way(wayId)
	s qsBox=$p(way,$c(1),1)
	s bllat=$p(way,$c(1),2)
	s bllon=$p(way,$c(1),3)
	s trlat=$p(way,$c(1),4)
	s trlon=$p(way,$c(1),5)
	i '$$qsOverlap(qsBox,bbox("root")) q 0
	i bllat'="",'$$overlap^quadString(bllat,bllon,trlat,trlon,bbox("bllat"),bbox("bllon"),bbox("trlat"),bbox("trlon")) q 0
	i bllat'="" q 1
	;
	s wayInBox=0
	s ndSeq=""
